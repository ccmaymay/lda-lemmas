...
\begin{table*}
    ...
    \begin{tabular}{r|l}
        ...
        lem & {\selectlanguage{russian}музей\wfa памятник коллекция библиотека выставка\wfb} \\
        non-lem & {\selectlanguage{russian}искусства музея\wfa картины выставки\wfb выставка\wfb} \\\hline
    \end{tabular}
    \caption{Manually-aligned topic pairs: the first topic in each pair
        ...
        \wfa denotes word forms derived from the same lemma and \wfb
        denotes word forms derived from the same lemma (but different
        from \wfa).  Note that the non-lemmatized
