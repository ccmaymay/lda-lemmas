...
symmetric prior.  On an
unfiltered vocabulary with an asymmetric prior designed to reduce
topic pollution
by stop words lemmatization renders no significant improvement.
Additionally, on length-truncated documents, neither the
filtered-symmetric nor
unfiltered-asymmetric models are benefitted by lemmatization.

Hand-aligned topics from the filtered-symmetric models learned on
full-length documents are displayed in Table~\ref{tab:topics}.
...
\begin{table}
    ...
    \end{tabular}
    \caption{Detection rate ($\DR$) for the non-lemmatized (non) and
...
We find that lemmatization yields a significant boost to the
interpretability of a topic model.  Specifically, in the configurations
studied, word intrusion performance is best when LDA is given a
symmetric prior and learned with a filtered vocabulary on full-length,
lemmatized documents.
Further, on lemmatized data in particular, we observe differences
...
We also note that interpretability is hindered by short
documents.  Our results thus confirm the intuition that topic modeling
in social media requires careful filtering and normalization of the text.
