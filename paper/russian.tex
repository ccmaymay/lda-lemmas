...
\newcommand{\MP}{\ensuremath{\textrm{MP}}}
...
\todo{give details of lemmatizer}

The model precision for all four configurations and the
p-values for one-sided model precision differences (testing our
hypothesis that the lemmatized models are more interpretable than the
unlemmatized models) are reported in
Table~\ref{tab:model-precision}.  Word intrusion performance benefits
...
\begin{table}
    \begin{tabular}{l|rr|r}
        experiment    &     raw $\MP$ &     lem $\MP$ &       p-value \\\hline
        base          &          0.54 &          0.52 &          0.61 \\
        \textbf{filt} & \textbf{0.50} & \textbf{0.65} & \textbf{0.02} \\
        trunc         &          0.37 &          0.37 &          0.50 \\
        trunc + filt  &          0.43 &          0.47 &          0.28 \\
    \end{tabular}
    \caption{Model precision ($\MP$) for the raw and lemmatized models
        and p-values for the test that the lemmatized model precision
        is greater than the unlemmatized (raw) model precision.
        Model precision is significantly benefitted by lemmatization on
