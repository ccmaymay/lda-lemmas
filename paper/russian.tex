...
lemmatizer \cite{schmid1994probabilistic}.\footnote{The tool is
  open-source and can be downloaded at {\small\tt
    http://www.cis.uni-muenchen.de/$\sim$schmid/\\tools/TreeTagger/}.}
...
\begin{table*}
    ...
    \caption{Manually-aligned topic pairs: the first topic in each pair
        ...
        similar topic in the non-lemmatized model. Note that the unlemmatized
        version of the first topic contains three inflections of the Russian word {\selectlanguage{russian}деревня} ({\em village})---this is redundant since each inflectional form
    arguably carries the identical topic information. 
...
\begin{figure}
    ...
    \caption{Examples of word intrusion interface on
        ...
        \ryan{We should make these pdfs! Otherewise, they look rather ugly.}
    }
...
released on 11/02/2015. \footnote{Available at
  https://dumps.wikimedia.org/ruwiki/20151102/ruwiki-20151102-pages-articles-multistream.xml.bz2.}
