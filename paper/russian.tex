...
evaluated using automated measures of model fit, topic quality, and
(induced) token cluster quality, as well as two heuristics designed to
identify the predictive words that were most affected by stemming.
TODO
Separately, the infinite-vocabulary LDA model has a prior
similar to an $n$-gram model~\cite{zhai2013}, which could be viewed as
loosely encoding beliefs of a concatenative morphology, but the effect
of that prior has not been analyzed in isolation.
